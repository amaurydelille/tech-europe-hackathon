...
- **Animated diagrams** (`gen_manim`): Manim-rendered scenes — equations being derived, vectors flowing through matmuls, graphs lighting up. Use this **only for math / CS / algorithm lessons** where a labeled animated diagram explains the idea better than imagery (e.g. softmax, attention, gradient descent, sorting). Read `src/backend/video_generation/manim_textbook.md` end-to-end before authoring your first Manim shot in a run. **If the lesson is math-heavy, you MUST use `gen_manim`** — at least one animated diagram per core concept. Imagery alone is not enough for math content. **Special case: if the topic is attention (transformer attention, self-attention, multi-head attention), center the entire video on explaining the mechanism, and put a real Manim animation of the attention computation (Q·Kᵀ → softmax → ·V, or similar) at the center of the video as the load-bearing visual.**
- **3D-globe map** (`gen_map`): slowly spinning Earth with countries highlighted in chosen colors, optional arrow between two lat/lon points (movement, access, trade route), optional city markers that light up at given times (sequential pings along a route), and optional country blink for mid-clip emphasis. Fast (~5–10 s). Use for any "where on Earth" beat — naming a region, showing a border or strait, tracing a route, locating an event. Pick `--lon`/`--lat`/`--zoom` to frame the relevant region. **Make sure to reach for this whenever the lesson has geographic stakes.** If the lesson names places, the video should show them on a map. For Strait of Hormuz videos specifically, use `gen_map` at least once. **Caveat: `gen_map` uses present-day borders (Natural Earth admin-0).** For historical lessons set in eras when borders differed (Roman Empire, pre-1945 Europe, colonial Africa, Soviet Union, Yugoslavia, etc.), do **not** use `gen_map` — generate a styled historical map as a `gen_image` still instead, then `animate_image` it. **Do not write a `sitecustomize.py` or set `CODEX_DISABLE_PLAYWRIGHT_GPU` to "work around" `gen_map` — those are not supported workarounds and will silently make the tool 80× slower. If `gen_map` errors, retry once with different parameters; if it still fails, fall back to an `animate_image` of a static map still.**
